...
# https://github.com/thu-ml/tianshou/blob/master/tianshou/policy/modelfree/pg.py
class PGPolicy_custom(BasePolicy):
    ...
    def learn(  # type: ignore
        ...
        for _ in range(repeat):
            for minibatch in batch.split(batch_size, merge_last=True):
                ...
                my_logprob = torch.log(torch.gather(1-(dist.probs*0.99), 1, act[:, None])).transpose(0, 1)
                #log_prob = dist.log_prob(act).reshape(len(ret), -1).transpose(0, 1)

                loss = -(my_logprob * ret).mean()
                loss.backward()
                self.optim.step()
